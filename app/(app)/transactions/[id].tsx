...
export default function TransactionDetailScreen() {
  ...
  const ngnAmount = isNaN(amountNum)
    ? 0
    : amountNum * (isNaN(rateNum) ? 1 : rateNum);
  ...
  return (
    <Screen refreshing={query.isRefetching} onRefresh={() => void query.refetch()}>
      ...
      {order.recipient?.memo ? (
        <Field label="Memo" value={order.recipient.memo} />
      ) : null}
      <Field
        label="Created"
      ...
      />
      {order.txHash ? (
        <Field label="Settlement tx" value={order.txHash} mono />
      ) : null}
      {order.gatewayId ? (
        <Field label="On-chain order" value={order.gatewayId} mono />
      ) : null}
    </Screen>
  );
}
...
function Field({
  ...
  return (
    <View className="py-3 border-b border-line-divider">
      <Text className="text-xs uppercase tracking-wider text-muted-subtle">
        {label}
      </Text>
      <Text
