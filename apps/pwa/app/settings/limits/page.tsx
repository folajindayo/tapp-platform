...
export default function SettingsLimitsPage() {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      // Limits are enforced from these values inside the debit transaction, so
      // saving them is the whole operation.
      //
      // There used to be a second half: convert each limit to USDC at a live
      // rate and push it on-chain, because the card's spending cap was a Move
      // object denominated in USDC. That made a limit somebody set in naira
      // depend on the exchange rate at the moment they saved it, and it failed
      // outright whenever the rate endpoint was unreachable. The card spends a
      // naira balance now and its limits are naira.
