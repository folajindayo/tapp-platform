...
func (uq *UserQuery) QueryProviderProfile() *ProviderProfileQuery {
	query := (&ProviderProfileClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTappCards chains the current query on the "tapp_cards" edge.
func (uq *UserQuery) QueryTappCards() *TappCardQuery {
	query := (&TappCardClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithVerificationToken tells the query-builder to eager-load the nodes that are connected to
// the "verification_token" edge. The optional arguments are used to configure the query builder of the edge.
...
// WithTappCards tells the query-builder to eager-load the nodes that are connected to
// the "tapp_cards" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithTappCards(opts ...func(*TappCardQuery)) *UserQuery {
	query := (&TappCardClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withTappCards = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withVerificationToken; query != nil {
	...
	if query := uq.withTappCards; query != nil {
		if err := uq.loadTappCards(ctx, query, nodes,
			func(n *User) { n.Edges.TappCards = []*TappCard{} },
			func(n *User, e *TappCard) { n.Edges.TappCards = append(n.Edges.TappCards, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadProviderProfile(ctx context.Context, query *ProviderProfileQuery, nodes []*User, init func(*User), assign func(*User, *ProviderProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadVerificationToken(ctx context.Context, query *VerificationTokenQuery, nodes []*User, init func(*User), assign func(*User, *VerificationToken)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.VerificationToken(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.VerificationTokenColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_verification_token
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_verification_token" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadTappCards(ctx context.Context, query *TappCardQuery, nodes []*User, init func(*User), assign func(*User, *TappCard)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.TappCard(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TappCardsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_tapp_cards
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_tapp_cards" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_tapp_cards" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
