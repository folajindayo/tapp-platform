...
func (c *Claude) Analyze(ctx context.Context, raw []byte, _ money.Amount) (*Result, error) {
	dhash, err := forensics.DHash(raw)
	if err != nil {
		return nil, err
	}
	...
	res := &Result{
		Mode:               "claude",
		Model:              c.model,
		ImageSHA256:        forensics.SHA256(raw),
		ImageDHash:         dhash,
	...
	total := money.Zero(money.NGN)
	for _, n := range res.Notes {
		sum, err := total.Add(n.Denomination)
		if err != nil {
			return nil, err
		}
		total = sum
...
func expand(p *claudeResponse, dhash string, warnings []string) ([]domain.Note, []string) {
	...
	for _, d := range denoms {
		kobo := money.Naira(int64(d))
		count := p.Counts[fmt.Sprintf("%d", d)]
		if !money.IsNairaDenomination(kobo) {
